...
pub async fn execute(config: Config, args: ConnectArgs) -> Result<()> {
    ...
    if status != AgentStatus::Running {
        return Err(anyhow!("Agent '{}' is not running. Start it with: crowdcontrol start {}", args.name, args.name));
    }
    ...
    if args.detach {
        ...
        print_success(&format!("Command started in background in agent '{}'", args.name));
    } else {
