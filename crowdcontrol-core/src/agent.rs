...
use crate::{Config, Agent, AgentStatus};
...
pub fn save_agent_metadata(config: &Config, agent: &Agent) -> Result<()> {
    let metadata_path = config.agent_workspace_path(&agent.name)
        .join(".crowdcontrol")
        .join("metadata.json");
    ...
    file.lock_exclusive()
        .with_context(|| format!("Failed to acquire lock on metadata file: {:?}", metadata_path))?;
...
pub fn load_agent_metadata(config: &Config, name: &str) -> Result<Agent> {
    ...
    debug!("Loading metadata for agent '{}' from {:?}", name, metadata_path);
    ...
    let metadata: AgentMetadata = serde_json::from_str(&json)
        .with_context(|| "Failed to parse agent metadata")?;
...
pub fn validate_agent_name(name: &str) -> Result<()> {
    ...
    if !name.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_') {
        return Err(anyhow!("Agent name can only contain alphanumeric characters, hyphens, and underscores"));
    }
...
pub fn clone_repository(
    ...
    if let Some(branch_name) = branch {
        ...
        info!("Cloning repository {} (branch: {}) to {:?}", repository, branch_name, target_path);
    } else {
